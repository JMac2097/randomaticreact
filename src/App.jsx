  ...
  return (
    <div style={{ padding: '20px', backgroundColor: theme === 'dark' ? '#333' : '#fff', color: theme === 'dark' ? '#fff' : '#333' }}>
      ...
      <section>
        ...
        <br />
        <FancyInput ref={fancyInputRef} placeholder="Fancy input with imperative handle" />
        <button onClick={() => fancyInputRef.current.focus()}>Focus Fancy Input</button>
        <button onClick={() => fancyInputRef.current.setValue('Hello from parent!')}>
      ...
      <section>
        <h2>8. useMemo</h2>
        <p>Expensive calculation result (only recalculated when count changes): {expensiveCalculation}</p>
      </section>
      ...
      <section>
        ...
        <p><strong>useImperativeHandle:</strong> Demonstrated with the FancyInput component above</p>
        <p><strong>useDebugValue:</strong> Used in the useWindowSize custom hook (visible in React DevTools)</p>
      </section>
    </div>
  );
}
